...
def calculateCentrePolygonPoints(nodesInfo,centroid,radius, middleConnectorSize): #find points of centre polygon, aligned correctly.
      polygonPoints = []
      for node in nodesInfo:
            ...
            polygonPoint = [(middleConnectorSize*radius)*coord for coord in node["unitVector"]] #multiply unit vector by a multiple of the radius so the shape will sit outside the circle
...
def findCircleConnectionPointProportion(node1, node2, radius, centroid): #Should be fixed
      ...
      #Above no longer always true as circleconnecting point not in the middle of those directions.
...
def findCircleConnectionPoint(node1, node2, radius, centroid): #Should be fixed
      #The curved line must connect to the point on the circle that is equidistant from the two node points (the closest one as there will be two).
      #The direction of this point on the circle from the centroid can be found by adding the unit vectors (the direction of the node from the centroid) of the two nodes together.
      resultantVector = [node1["unitVector"][i] + node2["unitVector"][i] for i in range(2)] #Doesn't produce a unit vector so the point isn't actually radius distance away.
      vectorMag = sqrt((resultantVector[0]**2) + (resultantVector[1])**2)
...
def calcCentreCurveVerts(nodeFrom,nodeTo, radius, centroid):
...
##      print(vector)
      ...
      #plt.plot(midpointOfPolygonPoints[0],midpointOfPolygonPoints[1], "go")
      vector = [midpointOfPolygonPoints[i] - circleConnectionPoint[i] for i in range(2)]
      vectorMag = sqrt((vector[0]**2) + (vector[1]**2))
      #print("vm",vectorMag)
      pointFrom = []
      controlPoint = [vectorMag*(-coord)*2 for coord in controlPointDirection]
      controlPoint = [controlPoint[i] + midpointOfPolygonPoints[i] for i in range(2)]
...
##      print(vector)
      ...
      vectorMag = sqrt((vector[0]**2) + (vector[1]**2))
      #print("vm",vectorMag)
      controlPoint = [vectorMag*(-coord)*2 for coord in controlPointDirection] # I don't think I can just use 2 anymore
      controlPoint = [controlPoint[i] + pointOnLine[i] for i in range(2)]
      #controlPoint = [controlPoint[i] - centroid[i] for i in range(2)]
      #pFrom = (0,122.46)
      #controlPoint = [controlPoint[i] + centroid[i] for i in range(2)] #Here is the problem.
      #controlPoint = [controlPoint[i] + pFrom[i] for i in range(2)]
      #plt.plot(controlPoint[0],controlPoint[1], "yo")
...
def drawEdge(radius, nodeRadius, nodesInfo, ax): #2 edgecase and 1
      ...
      for nodeFromIndex in range(len(nodesInfo)-3):
...
nodesInfo = setUpNodeDicts([[-260,220],[260,220],[260,-220],[-260,-150]])
radius = 20
